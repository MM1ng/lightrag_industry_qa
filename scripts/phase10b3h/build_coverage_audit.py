...
def disagreement(rows,mapping_rows):
    ...
    for row in rows:
        ...
        out.append({"question_id":row["question_id"],"split":row["split"],"external_failure":"unsupported_or_citation","classification":cats,"claims":bound,"golden_expected_evidence":g.get("expected_evidence",[]),"golden_expected_answer_points":g.get("expected_answer_points",[]),"response_answer":row["response"].get("answer"),"response_citations":row["response"].get("citations",[]),"comparison_note":"Claim-citation mapping is structurally present, but cited candidate chunk differs from fixed Candidate Golden mapping; support requires object/parameter/value/unit/condition review."})
    return out
