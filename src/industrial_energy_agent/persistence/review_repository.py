...
from industrial_energy_agent.domain.models import RiskReview, WorkOrderReview
...
class ReviewRepository:
    ...
    def create_risk_review(
        ...
        *,
        ...
        if existing is not None:
            review = self._risk_from_row(existing)
            if (
                review.request_id != request_id
                or review.conversation_id != conversation_id
                or review.risk_category != risk_category
                or review.restricted_answer_hash != restricted_answer_hash
            ):
    ...
    def create_work_order_review(
        ...
        *,
        ...
        if existing is not None:
            review = self._work_order_from_row(existing)
            if review.work_order_id != work_order_id or review.request_id != request_id:
                raise DomainValidationError("work order review idempotency key conflicts")
            return review
        if target is None:
            raise DomainValidationError("work order review requires a real work order draft")
